...
export default function MainNav({
  ...
  return (
    <MainNavContext.Provider value={{ toggleMenu }}>
      <nav
        ref={ref}
        className={`fixed left-0 top-0 h-dvh w-72 border-r-[1px] border-r-primary-50 py-3 backdrop-blur-md dark:border-r-secondary-400 ${isActive ? "translate-x-0" : "-translate-x-full"} flex flex-col transition-transform xl:static xl:h-full xl:translate-x-0 xl:border-none`}
      >
        <Logo className="px-6" />
...
function NavRow({ children, to, className }: NavRowProps) {
  ...
  return (
    <li className="rounded-md">
      <NavLink
        to={to}
        className={`flex w-full items-center gap-4 rounded-md bg-primary-600 px-4 py-2 text-2xl text-white transition-colors hover:bg-primary-800 hover:text-white ${className ? className : ""}`}
        onClick={toggleMenu}
      >
        {children}
      </NavLink>
